...
def save_video(video_name, y, output_name):
    video = VideoFileClip(video_name)
    fps = video.fps
    length = int(video.fps * video.duration)
    expanded_y = expand_y(y, length, fps)
    final_vid = concatenate_videoclips([video.subclip(start, end) for (start, end) in frame_to_interval(expanded_y, fps)])
    final_vid.to_videofile(output_name, fps=fps)
